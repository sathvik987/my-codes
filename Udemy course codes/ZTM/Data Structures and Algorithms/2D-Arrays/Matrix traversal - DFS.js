...
const dfs2 = (matrix, row, col, seen, values) => {
    if (row < 0 || col < 0 || row === matrix.length || col === matrix[0].length || seen.has(String(row) + String(col))) {
        return values
    }
    values.push(matrix[row][col]);
    seen.add(String(row) + String(col))
    dfs2(matrix, row - 1, col, seen, values)
    dfs2(matrix, row, col + 1, seen, values)
    dfs2(matrix, row + 1, col, seen, values)
    dfs2(matrix, row, col - 1, seen, values)
    return values
}

console.log(dfs2(testMatrix, 0, 0, new Set(), []));